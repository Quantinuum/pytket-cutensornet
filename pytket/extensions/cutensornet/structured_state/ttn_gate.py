...
class TTNxGate(TTN):
    ...
    def _apply_2q_unitary(self, unitary: cp.ndarray, q0: Qubit, q1: Qubit) -> TTNxGate:  # noqa: PLR0915
        ...
        self._truncate_path(list(reversed(bonds_to_q1)), bonds_to_q0)
    ...
    def _truncate_path(
        ...
        """Truncate all bonds in the lists according to the policy chosen by the user.
        ...
        """
        self._logger.debug("Starting sequential truncation.")
        ...
        # Find out which one is the bond with largest dimension
        if self._cfg.truncation_fidelity < 1:
            max_dim = 0
            truncate_more_at = truncation_bonds[0]
            for address in truncation_bonds:
                this_dim = self.get_dimension(address, DirTTN.PARENT)
                if max_dim < this_dim:
                    max_dim = this_dim
                    truncate_more_at = address

        # Truncate each bond in the path between the qubits
        for i, bond_address in enumerate(truncation_bonds):
            ...
            if self._cfg.truncation_fidelity < 1 and truncate_more_at == bond_address:
                ...
                self._logger.debug(
                    f"Truncating more at address {truncate_more_at}"  # noqa: G004
                )
                self._logger.debug(
        ...
        self._logger.debug(
            "Finished sequential truncation. "  # noqa: G004
